...
public class FullPrunedBlockChain extends AbstractBlockChain {
	 ...
	 */
	public FullPrunedBlockChain(Context context, Wallet wallet, FullPrunedBlockStore blockStore) throws
			BlockStoreException {
	 ...
	 * {@link Wallet#loadFromFile(File, WalletExtension...)}
	 */
	public FullPrunedBlockChain(NetworkParameters params, Wallet wallet, FullPrunedBlockStore blockStore) throws
			BlockStoreException {
	 ...
	 */
	public FullPrunedBlockChain(Context context, List<Wallet> listeners, FullPrunedBlockStore blockStore) throws
			BlockStoreException {
	 ...
	 * is redundant（多余的） because you know the connected node won't relay bad data to you. In that case it's safe
	 * to set this to false and obtain a significant speedup.
	 */
	...
	protected TransactionOutputChanges connectTransactions(int height, Block block)
		...
		try {
			if (!params.isCheckpoint(height)) {
				...
				// checkpoints list and we therefore only check non-checkpoints for duplicated transactions here. See
				// the
				// BIP30 document for more details on this: https://github
				// .com/bitcoin/bips/blob/master/bip-0030.mediawiki
				for (Transaction tx : block.transactions) {
					final Set<VerifyFlag> verifyFlags = params.getTransactionVerificationFlags(block, tx,
							getVersionTally(), height);
					...
					if (verifyFlags.contains(VerifyFlag.P2SH)) // We already check non-BIP16 sigops in Block
			...
			for (final Transaction tx : block.transactions) {
				...
				if (!isCoinBase) {
					...
					for (int index = 0; index < tx.getInputs().size(); index++) {
						...
						if (prevOut == null)
							throw new VerificationException("Attempted to spend a non-existent or already spent " +
									"output!");
						...
						if (prevOut.isCoinbase()) {
							if (height - prevOut.getHeight() < params.getSpendableCoinbaseDepth()) {
								throw new VerificationException("Tried to spend coinbase at depth " + (height -
										prevOut.getHeight()));
							}
						}
				...
				if (!isCoinBase && runScripts) {
					...
					FutureTask<VerificationException> future = new FutureTask<>(new Verifier(tx, prevOutScripts,
							verifyFlags));
					scriptVerificationExecutor.execute(future);
					listScriptVerificationResults.add(future);
				}
			}
			if (totalFees.compareTo(params.getMaxMoney()) > 0 || block.getBlockInflation(height).add(totalFees)
					.compareTo(coinbaseValue) < 0)
			...
			for (Future<VerificationException> future : listScriptVerificationResults) {
				...
				try {
					...
					throw new VerificationException("Bug in Script.correctlySpends, likely script malformed in some " +
							"new and interesting way.", thrownE);
				}
				if (e != null)
					throw e;
			}
		...
		try {
			...
			if (transactions != null) {
				...
				for (final Transaction tx : transactions) {
					...
					if (!isCoinBase) {
						for (int index = 0; index < tx.getInputs().size(); index++) {
							...
							final UTXO prevOut = blockStore.getTransactionOutput(in.getOutpoint().getHash(),
									in.getOutpoint().getIndex());
							if (prevOut == null)
								throw new VerificationException("Attempted spend of a non-existent or already spent " +
										"output!");
							if (prevOut.isCoinbase() && newBlock.getHeight() - prevOut.getHeight() < params
									.getSpendableCoinbaseDepth())
								throw new VerificationException("Tried to spend coinbase at depth " + (newBlock
										.getHeight() - prevOut.getHeight()));
							valueIn = valueIn.add(prevOut.getValue());
							if (verifyFlags.contains(VerifyFlag.P2SH)) {
								if (ScriptPattern.isPayToScriptHash(prevOut.getScript()))
									sigOps += Script.getP2SHSigOpCount(in.getScriptBytes());
					...
					// All values were already checked for being non-negative (as it is verified in Transaction
					// .verify())
					// but we check again here just for defence in depth. Transactions with zero output value are OK.
					if (valueOut.signum() < 0 || valueOut.compareTo(params.getMaxMoney()) > 0)
						throw new VerificationException("Transaction output value out of range");
				...
				if (totalFees.compareTo(params.getMaxMoney()) > 0 ||
						newBlock.getHeader().getBlockInflation(newBlock.getHeight()).add(totalFees).compareTo
								(coinbaseValue) < 0)
					throw new VerificationException("Transaction fees out of range");
				...
				for (Future<VerificationException> future : listScriptVerificationResults) {
					...
					try {
						...
						log.error("Script.correctlySpends threw a non-normal exception: " + thrownE.getCause());
						throw new VerificationException("Bug in Script.correctlySpends, likely script malformed in " +
								"some new and interesting way.", thrownE);
					}
					if (e != null)
						throw e;
				}
